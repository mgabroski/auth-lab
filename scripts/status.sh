...
probe_url "Control Plane API shim" "http://localhost:3002/api/health"
probe_url "Control Plane accounts API" "http://localhost:3002/api/cp/accounts"
...
echo "- localhost:3002 is the separate internal Control Plane app in host-run mode."
echo "- localhost:3002/api/* is a CP Next.js shim to localhost:3001; it requires backend CP_ENABLED=true and CP_AUTH_MODE=none locally."
