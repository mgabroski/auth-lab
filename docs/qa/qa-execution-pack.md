...
| Setup Guidance   | Settings-native workspace setup banner, `/admin/settings` overview, real `/admin/settings/access` acknowledge flow, real `/admin/settings/account` card saves, Communications placeholder route, Permissions absence                            | Local       |
...
| Settings closure | `/settings/bootstrap`, `/settings/overview`, `/settings/access`, `/settings/access/acknowledge`, `/settings/account`, the three Account card save routes, synchronous CP cascade, persisted aggregate/section state, and honest route treatment | Local       |
