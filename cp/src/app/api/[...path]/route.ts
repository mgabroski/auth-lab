 ...
 *
 * WHY:
 * - CP browser code must use same-origin /api/* paths, not hardcoded backend origins.
 * - This Route Handler proxies those requests to the backend while preserving
 *   forwarded-header context for request correlation.
 * - Mirrors the pattern in frontend/src/app/api/[...path]/route.ts.
 *
 * IMPORTANT:
 ...
 * - Use INTERNAL_API_URL env var (server-side) or this proxy route (client-side).
 */
